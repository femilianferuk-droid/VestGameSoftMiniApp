...
# Картинки лежат прямо в корне проекта (рядом с app.py) — без отдельной
# папки static. Отдаём их по пути /img/<filename>.
STATIC_DIR = BASE_DIR
HERO_IMG = "vest-hero.webp"
AUTO_IMG = "vest-automation.webp"
IMG_DIR_NAME = "img"  # префикс URL: /img/vest-hero.webp
...
def init_db():
    ...
    Безопасно вызывать много раз — CREATE TABLE IF NOT EXISTS."""
    # Если прошлый запуск упал до создания таблиц, пробуем ещё раз
    try:
        db_query("SELECT 1")
    except Exception as e:
        logger.warning("DB not reachable yet, skipping init_db: %s", e)
        return
    db_exec("""
...
def _upsert_user(user: dict) -> None:
    """Сохраняет/обновляет пользователя в БД (best-effort)."""
    try:
        db_exec(
...
def require_auth(view):
    """Строгая проверка: нет валидной initData — отдаём LOGIN_PAGE.
    Используется для API-эндпоинтов и для страниц, требующих гарантированной авторизации."""
    @wraps(view)
...
def optional_auth(view):
    """Мягкая проверка: пытаемся достать user из initData, но если не получилось —
    всё равно рендерим страницу. Используется для страничных роутов (/, /account/...,
    /chat/...), потому что initData доступен только в JS (Telegram.WebApp.initData),
    а не в начальном HTTP-запросе от WebView. Реальные данные грузятся с клиента
    через /api/* эндпоинты, которые защищены @require_auth и вернут 401,
    если initData нет/невалидна — JS-хендлер покажет LOGIN_PAGE.
    """
    @wraps(view)
    def wrapper(*args, **kwargs):
        user = get_user_from_request()
...
@app.route(f"/{IMG_DIR_NAME}/<path:filename>")
def static_files(filename: str):
    """Отдача картинок из корня проекта по URL /img/<filename>.
    Безопасный путь: проверяем, что итоговый файл лежит внутри BASE_DIR."""
...
def accounts_page(user):
    # Список аккаунтов грузится клиентом через /api/accounts —
    # initData есть только в JS, в начальном HTTP-запросе её нет.
    return _render("accounts", user=user, accounts=[])


@app.route("/account/<int:account_id>")
@optional_auth
def chats_page(user, account_id: int):
    # account_id берём из URL, реальные данные (phone, proxy, …) подтягивает
    # клиент через /api/account/<id>/info. Если user не авторизован —
    # API вернёт 401 и JS покажет LOGIN_PAGE.
    return _render("chats", user=user, account={"id": account_id, "phone": "…"})
...
def chat_page(user, account_id: int, chat_id: int):
    return _render(
        "chat", user=user,
        account={"id": account_id, "phone": "…"},
        chat_id=chat_id, title=f"Chat {chat_id}",
    )
...
@require_auth
def api_accounts(user):
    """Список аккаунтов текущего пользователя. Грузится с клиента,
    чтобы initData (которая доступна только в JS) попала в запрос."""
    rows = db_query(
    ...
    return jsonify(ok=True, accounts=[{
        ...
        "warming_enabled": bool(r.get("warming_enabled")),
        "proxy_label": r.get("proxy_label") or "",
...
.hdr .brand-mark{width:30px;height:30px;border-radius:8px;flex-shrink:0;object-fit:cover;
  box-shadow:0 1px 3px rgba(0,0,0,.15);background:#fff}
...
.empty-hero{padding:24px 16px}
.empty-hero .hero-img{width:100%;max-width:320px;border-radius:18px;display:block;
  margin:0 auto 18px;box-shadow:0 6px 24px rgba(0,0,0,.15)}
.empty-hero .empty-title{font-size:17px;font-weight:600;color:var(--text);
  margin:6px 0 4px}
...
(function(){
  ...
  window.__post = async (url, body) => {
    const r = await fetch(url, {
      method:'POST',
      headers:{'Content-Type':'application/json','X-Telegram-Init-Data': window.__initData || ''},
      body: JSON.stringify(body || {})
    });
    if(r.status === 401){ window.__unauth(); return { _unauth: true, status: 401 }; }
    return r;
  };
  ...
  // Показываем LOGIN_PAGE при 401 от любого API-запроса. Показываем
  // ровно один раз — дальше не спамим, чтобы не моргало.
  window.__unauthShown = false;
  window.__unauth = () => {
    if(window.__unauthShown) return;
    window.__unauthShown = true;
    const wa2 = window.Telegram && Telegram.WebApp;
    const inTg = !!(wa2 && wa2.initData);
    const msg = inTg
      ? 'Сессия невалидна. Открой мини-приложение заново через @VestGameSoftbot.'
      : 'Это приложение доступно только из Telegram. Открой его через @VestGameSoftbot → кнопка с приложением.';
    const html = '<div style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif;'
      + 'background:#17212b;color:#fff;margin:0;padding:20px;min-height:100vh;'
      + 'display:flex;align-items:center;justify-content:center;box-sizing:border-box">'
      + '<div style="max-width:380px;width:100%;background:#0e1621;padding:24px;border-radius:18px;'
      + 'box-shadow:0 8px 32px rgba(0,0,0,.3);text-align:center">'
      + '<div style="font-size:40px;margin-bottom:10px">🔒</div>'
      + '<h1 style="font-size:20px;margin:0 0 10px">{{ app_name }}</h1>'
      + '<p style="color:#708499;line-height:1.5;font-size:14px;margin:6px 0">' + msg + '</p>'
      + '<button id="__unauthBtn" style="display:inline-block;margin-top:18px;padding:10px 20px;'
      + 'background:linear-gradient(135deg,#5fb3f0,#2481cc);color:#fff;border-radius:10px;'
      + 'font-weight:600;font-size:14px;border:none;cursor:pointer">Закрыть</button>'
      + '</div></div>';
    document.body.innerHTML = html;
    const b = document.getElementById('__unauthBtn');
    if(b) b.onclick = () => { try{ wa2 && wa2.close(); }catch(e){} };
  };
})();
</script>
...
<style>
  body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;
    ...
    background:#17212b;color:#fff;margin:0;padding:20px;box-sizing:border-box}
  .box{max-width:380px;width:100%;background:#0e1621;padding:24px;border-radius:18px;
    box-shadow:0 8px 32px rgba(0,0,0,.3);text-align:center}
  .hero{width:100%;border-radius:14px;margin-bottom:18px;display:block}
  h1{font-size:20px;margin:0 0 10px}
  p{color:#708499;line-height:1.5;font-size:14px;margin:6px 0}
  strong{color:#7cc4ff}
  .btn{display:inline-block;margin-top:18px;padding:10px 20px;
    background:linear-gradient(135deg,#5fb3f0,#2481cc);color:#fff;
    border-radius:10px;text-decoration:none;font-weight:600;font-size:14px;border:none;cursor:pointer}
  ol{text-align:left;color:#708499;font-size:13px;line-height:1.6;
    margin:14px auto;max-width:280px}
  ol li{margin:4px 0}
</style></head>
<body><div class="box">
  <img class="hero" src="/img/vest-hero.webp" alt="Vest Chat" onerror="this.style.display='none'">
  <h1>🔒 {{ app_name }}</h1>
  <p>Это приложение доступно только из Telegram.<br>Открой его через бота
  <strong>@VestGameSoftbot</strong> → кнопка с приложением.</p>
  <ol>
    <li>Открой Telegram и найди бота <strong>@VestGameSoftbot</strong></li>
    <li>Нажми <strong>Menu → кнопка с приложением</strong> (или синюю кнопку внизу)</li>
    <li>Приложение откроется внутри Telegram</li>
  </ol>
  <button class="btn" onclick="Telegram.WebApp && Telegram.WebApp.close()">Закрыть</button>
</div></body></html>"""
...
{% block body %}
<div class="hdr">
  <img class="brand-mark" src="/img/vest-automation.webp" alt="Vest" onerror="this.style.display='none'">
  <div class="title">{{ app_name }}</div>
  <div class="actions"><div class="icon-btn" title="Помощь" onclick="__toast('Кликни на аккаунт → список чатов → открой чат')">?</div></div>
</div>
<div class="scroll" id="accountList">
  <div class="empty"><div class="big">⏳</div><p>Загружаю аккаунты…</p></div>
</div>
{% endblock %}
{% block scripts %}
<script>
const FALLBACK_AVATAR_ACC = (name) => {
  const colors = [
...
const fmtPhone = (p) => {
  if(!p) return '—';
  p = String(p).trim();
  if(p.startsWith('+') && p.length >= 7) return p.slice(0,5) + '***' + p.slice(-3);
  return p;
};
...
async function loadAccounts(){
  ...
  try {
    const r = await __get('/api/accounts');
    if(r && r._unauth) return;  // global handler уже показал LOGIN_PAGE
    const j = await r.json();
    if(!j.ok){
      list.innerHTML = '<div class="empty"><div class="big">⚠</div><p>'+escapeHtml(j.error||'Ошибка')+'</p></div>';
      return;
    }
    if(!j.accounts.length){
      list.innerHTML = `
        <div class="empty empty-hero">
          <img class="hero-img" src="/img/vest-hero.webp" alt="Vest Chat" onerror="this.style.display='none'">
          <p class="empty-title">Нет аккаунтов</p>
          <p style="margin-top:6px;font-size:12px">Добавь аккаунт в @VestGameSoftbot</p>
        </div>`;
      return;
    }
    let html = '<div class="section-title">Твои аккаунты ('+j.accounts.length+')</div>';
    for(const a of j.accounts){
      const phone = a.phone || '';
      const tail = phone.replace(/\\D/g,'').slice(-2) || '?';
      const status = a.is_active
        ? '<span style="color:#1ba35b">● online</span>'
        : '<span style="color:#d63838">● offline</span>';
      const proxy = a.proxy_label ? ' · 🔗 '+escapeHtml(a.proxy_label) : '';
      const warming = a.warming_enabled ? ' · 🔥 прогрев' : '';
      html += `
        <a class="account" href="/account/${a.id}" style="text-decoration:none;color:inherit">
          <div class="av" style="background:${FALLBACK_AVATAR_ACC(phone)}">${escapeHtml(tail)}</div>
          <div class="meta">
            <div class="name">${escapeHtml(fmtPhone(phone))}</div>
            <div class="sub">${status}${proxy}${warming}</div>
          </div>
          <div class="arrow">›</div>
        </a>`;
    }
    ...
    list.innerHTML = '<div class="empty"><div class="big">⚠</div><p>'+escapeHtml(e.message||String(e))+'</p></div>';
  }
}
...
def _safe_init_db():
    """Запускает init_db() в try/except, чтобы не валить процесс при
    недоступной БД — приложение должно подняться, чтобы отдать
    пользователю понятную страницу вместо «Internal Server Error»."""
    try:
        init_db()
        logger.info("init_db: tables OK")
    except Exception as e:
        logger.warning("init_db failed at startup: %s", e)


# Запускаем миграции сразу при импорте модуля, чтобы gunicorn/uvicorn
# тоже подхватывали создание таблиц (раньше init_db был только в __main__).
_safe_init_db()


# --- Глобальный обработчик 500: возвращает аккуратную HTML-страницу ---
# Без него Flask отдаёт голую «Internal Server Error», что и видел юзер
# при открытии мини-аппа.
...
<title>Vest Chat — ошибка</title>
<script src="https://telegram.org/js/telegram-web-app.js"></script>
<style>
  body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;
    background:#17212b;color:#fff;margin:0;padding:24px;
    min-height:100vh;display:flex;align-items:center;justify-content:center}
  .card{max-width:420px;background:#0e1621;padding:28px;border-radius:18px;
    box-shadow:0 8px 32px rgba(0,0,0,.3);text-align:center}
  img{width:100%;border-radius:14px;margin-bottom:18px;display:block}
  h1{font-size:18px;margin:0 0 8px}
  p{color:#708499;font-size:14px;line-height:1.5;margin:6px 0}
  code{background:#1c2733;padding:2px 6px;border-radius:6px;color:#7cc4ff;
...
<div class="card">
  <img src="/img/""" + HERO_IMG + """" alt="Vest Chat">
  <h1>⚠️ Не удалось открыть чат</h1>
  <p>Мини-приложение сейчас на перезагрузке.<br>Попробуй открыть его заново через пару секунд.</p>
  <p>Если ошибка повторяется — напиши <code>@VestGameSoftbot</code></p>
  <a class="btn" href="javascript:Telegram.WebApp.close()">Закрыть</a>
</div>
</body></html>""",
        500,
    )
...
def _handle_404(e):
    return (
        """<!doctype html><html><head><meta charset="utf-8">
<title>404</title><style>body{font-family:sans-serif;background:#17212b;
color:#fff;display:flex;align-items:center;justify-content:center;height:100vh;
margin:0}div{text-align:center;background:#0e1621;padding:32px;border-radius:18px}
h1{font-size:48px;margin:0 0 8px}p{color:#708499}</style></head><body>
<div><h1>404</h1><p>Страница не найдена</p>
<a href="/" style="color:#5fb3f0">← На главную</a></div></body></html>""",
        404,
    )
...
def _handle_unhandled(e):
    """Ловим всё, что не поймали в try/except — чтобы 502/Internal Server Error
    никогда не показался пользователю голым текстом."""
    return _handle_500(e)


if __name__ == "__main__":
    _safe_init_db()  # повторно — на случай, если первый запуск был до сети
